...
def greedy_set_cover(history, k=15, max_val=25):
    ...
    # Se ainda faltar preencher para atingir k números, complete com os mais frequentes do histórico
    if len(selected) < k:
        flat = [num for sub in history for num in (sub[0] if isinstance(sub[0], list) else sub)]
        if flat:
            counts = pd.Series(flat).value_counts()
            for val in counts.index:
                if val not in selected and len(selected) < k and 1 <= val <= max_val:
                    selected.append(int(val))
                    
    # Se ainda assim não atingir k, preencha sequencialmente
    if len(selected) < k:
