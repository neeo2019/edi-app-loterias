...
# -----------------------------------------------------------------------------
# INTEGRAÇÃO VIA API (CONEXÃO EM TEMPO REAL COM A CAIXA)
# -----------------------------------------------------------------------------
# Mapeamento de endpoints oficiais da API de Loterias
API_ENDPOINTS = {
    "Mega-Sena": "megasena",
    "Lotofácil": "lotofacil",
    "Quina": "quina",
...
# Dados de Fallback (Caso ocorra falha de conexão na API)
LOTERIAS_CONFIG_DEFAULT = {
    "Mega-Sena": {
        "cor": "#00a859",
        "total_dezenas": 60,
        "qtd_jogo": 6,
        "ultimo_concurso": "2830",
    ...
    "Dupla Sena": {
        ...
        "total_dezenas": 50,
        "qtd_jogo": 6,
        "ultimo_concurso": "2705",
        "data_ultimo": "25/08/2026",
        "dezenas_ultimo": [13, 14, 17, 39, 42, 46],
        "dezenas_ultimo_2": [4, 11, 23, 31, 37, 48],
        "validos": list(range(1, 51))
    }
}

@st.cache_data(ttl=300)  # Cache de 5 minutos para otimizar requisições
def buscar_dados_caixa_api():
    """Busca resultados atualizados diretamente do serviço oficial de Loterias."""
    config_atualizada = LOTERIAS_CONFIG_DEFAULT.copy()
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
    }
    
    for nome_loteria, slug in API_ENDPOINTS.items():
        url = f"https://servicebus2.caixa.gov.br/portaldeloterias/api/{slug}"
        try:
            res = requests.get(url, headers=headers, timeout=5)
            if res.status_code == 200:
                data = res.json()
                config_atualizada[nome_loteria]["ultimo_concurso"] = str(data.get("numero", config_atualizada[nome_loteria]["ultimo_concurso"]))
                config_atualizada[nome_loteria]["data_ultimo"] = data.get("dataApuracao", config_atualizada[nome_loteria]["data_ultimo"])
                
                dezenas = [int(d) for d in data.get("listaDezenas", [])]
                if dezenas:
                    config_atualizada[nome_loteria]["dezenas_ultimo"] = dezenas
                
                # Tratamento exclusivo para o 2º sorteio da Dupla Sena
                if nome_loteria == "Dupla Sena" and "listaDezenasSegundoSorteio" in data:
                    dezenas_2 = [int(d) for d in data.get("listaDezenasSegundoSorteio", [])]
                    if dezenas_2:
                        config_atualizada[nome_loteria]["dezenas_ultimo_2"] = dezenas_2
        except Exception:
            # Em caso de instabilidade da API da Caixa, mantém os dados do dicionário local
            pass
            
    return config_atualizada

# Carrega e sincroniza o dicionário de loterias
LOTERIAS_CONFIG = buscar_dados_caixa_api()
...
def gerar_palpites_otimizados(loteria_nome):
    ...
    for i in range(3):
        if loteria_nome == "Lotomania":
            ...
            anteriores_15 = c["dezenas_ultimo"][:15]
            ...
            qtd_rep = min(9, len(dezenas_anteriores))
            repetidas_9 = list(np.random.choice(dezenas_anteriores, qtd_rep, replace=False))
...
st.caption("Dados sincronizados em tempo real diretamente da Caixa Econômica Federal.")
...
if st.button("ATUALIZAR RESULTADOS DESDE A CAIXA"):
    st.cache_data.clear()
    st.toast("Conexão realizada com a Caixa. Resultados atualizados!")
    st.rerun()
...
if st.session_state.jogos_salvos:
    for idx, jogo in enumerate(st.session_state.jogos_salvos):
        ...
        if jogo["loteria"] == "Dupla Sena":
            dezenas_sorteadas_2 = set(lot_cfg.get("dezenas_ultimo_2", []))
