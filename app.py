...
# -----------------------------------------------------------------------------
# CONFIGURAÇÕES E BANCO DE DADOS DAS LOTERIAS
# -----------------------------------------------------------------------------
LOTERIAS_CONFIG = {
    "Mega-Sena": {
        "cor": "#00a859",
        ...
        "filtros_validos": [n for n in range(1, 61) if n % 10 not in [0, 2, 6]]
    },
    "Lotofácil": {
        "cor": "#930089",
        "total_dezenas": 25,
        ...
        "filtros_validos": list(range(1, 26))
    },
    "Quina": {
        "cor": "#260085",
        "total_dezenas": 80,
        ...
        "filtros_validos": list(range(1, 81))
    },
    "Lotomania": {
        "cor": "#f78200",
        "total_dezenas": 100,
        ...
        "filtros_validos": list(range(1, 101))
    },
    "Dupla Sena": {
        "cor": "#a6121f",
        "total_dezenas": 50,
        ...
        "filtros_validos": list(range(1, 51))
    }
}
...
# -----------------------------------------------------------------------------
# FUNÇÃO DE GERAÇÃO DE PALPITES COM REFRESH AUTOMÁTICO
# -----------------------------------------------------------------------------
def gerar_palpites(loteria_nome):
    np.random.seed()
    c = LOTERIAS_CONFIG[loteria_nome]
    validos = c["filtros_validos"]
    qtd = c["qtd_jogo"]
    ...
    for i in range(3):
        if loteria_nome == "Lotofácil":
            ...
            dinamicas = list(np.random.choice(range(35, 101), 16, replace=False))
            jogo = sorted(esqueleto + dinamicas)
            dezena_ouro = 7
        else:
            jogo = sorted(list(np.random.choice(validos, qtd, replace=False)))
            dezena_ouro = jogo[-1]
...
if 'palpites_atuais' not in st.session_state or st.session_state.get('loteria_ativa') != st.session_state.loteria_selecionada:
    st.session_state.palpites_atuais = gerar_palpites(st.session_state.loteria_selecionada)
    st.session_state.loteria_ativa = st.session_state.loteria_selecionada

# -----------------------------------------------------------------------------
# CSS CUSTOMIZADO TEMA ESCURO (FIEL AO DESIGN ORIGINAL)
# -----------------------------------------------------------------------------
st.markdown(f"""
    <style>
    .stApp {{
...
# -----------------------------------------------------------------------------
# CABEÇALHO E NAVEGAÇÃO ENTRE LOTERIAS
# -----------------------------------------------------------------------------
...
for idx, (nome_lot, d) in enumerate(LOTERIAS_CONFIG.items()):
    with cols_nav[idx]:
        if st.button(nome_lot, key=f"btn_nav_{nome_lot}"):
            ...
            st.session_state.palpites_atuais = gerar_palpites(nome_lot)
...
if st.button("ATUALIZAR RESULTADOS DESDE A CAIXA"):
    st.toast("Concursos atualizados com sucesso!")
st.markdown('</div>', unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# BOTÕES DE AÇÃO: MANUAL E REGERAR
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# EXIBIÇÃO DOS 3 MELHORES PALPITES
# -----------------------------------------------------------------------------
...
for idx, p in enumerate(st.session_state.palpites_atuais):
    ...
    <div class="box-dark">
        ...
        <div style="font-size:12px; color:#94a3b8; margin-bottom:10px;">
            Ouro {p['ouro']} · Soma {sum(p['dezenas'])} · P/I Equilibrio
        </div>
        <div>
            ...
            <span class="badge-tag">Monte Carlo</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
# JOGOS SALVOS E CONFERÊNCIA AUTOMÁTICA
# -----------------------------------------------------------------------------
...
if not st.session_state.jogos_salvos:
    ...
    for idx, jogo in enumerate(st.session_state.jogos_salvos):
        # Lógica de conferência com as dezenas do último concurso ativo
        ...
        with c_conf:
            ...
            if st.button("Conferir", key=f"btn_conf_{idx}"):
                st.toast(f"Conferência: {acertos} acertos registrados para este jogo!")
            st.markdown('</div>', unsafe_allow_html=True)
